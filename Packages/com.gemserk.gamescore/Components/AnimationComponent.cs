...
namespace Game.Components
{
    ...
    public struct AnimationComponent : IEntityComponent
    {
        ...
        {
            var animationIndex = animationsAsset.GetAnimationIndexByName(animation);
            Assert.IsTrue(animationIndex >= 0, $"Couldn't find {animation}");
            Play(animationsAsset.GetAnimationIndexByName(animation), loops);
        }

        public bool IsPlaying(string animationName)
        {
            return currentAnimation == animationsAsset.GetAnimationIndexByName(animationName);
        }

        public bool HasAnimation(string animationName)
        {
            return animationsAsset.HasAnimation(animationName);
        }
